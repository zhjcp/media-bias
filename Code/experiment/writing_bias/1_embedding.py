...
def train_model_by_base_corpus(media_list, random_seed_pretrain=None):
    ...
    # (3) 保存词向量模型
    print('start save')
    save_dir = '/home/newsgrid/zhuhua/NHB/data/embedding/word2vec/model_epoch_coarse-grain/' + 'base'
...
def fine_tune_model_coarse_grain(media_name, random_seed_pretrain, random_seed_finetune, epoch_list):
    ...
    for epoch in epoch_list:
        ...
        # (3) 保存
        save_dir = '/home/newsgrid/zhuhua/NHB/data/embedding/word2vec/model_epoch_coarse-grain/' + media_name
        if not os.path.exists(save_dir):
            os.makedirs(save_dir, exist_ok=True)
...
def fine_tune_model_coarse_grain_wrt_year(media_name, random_seed_pretrain, random_seed_finetune, epoch_list, year_set):
    ...
    for epoch in epoch_list:
        ...
        save_dir = '/home/newsgrid/zhuhua/NHB/data/embedding/word2vec/model_epoch_coarse-grain_wrt_year/' + media_name + '/' + year_set_info
        if not os.path.exists(save_dir):
            os.makedirs(save_dir, exist_ok=True)
        model.save(os.path.join(save_dir, media_name+'_rs_'+str(random_seed_pretrain)+'_rs_'+str(random_seed_finetune)+'_finetune_'+str(epoch)+'.model'))
...
def train_model_alone_by_mediaName(media_name, year_set=None):
    ...
    # (3) 保存词向量模型
    print('start save')
    save_dir = '/home/newsgrid/zhuhua/NHB/data/embedding/word2vec/model_alone/' + media_name
    if not os.path.exists(save_dir):
        os.makedirs(save_dir, exist_ok=True)
    if year_set is None:
...
def evaluate_model_by_word_analogies(model):
    ...
    :return: None
    """
    test_dataset_path = '/home/newsgrid/zhuhua/NHB/data/embedding/word2vec/media/test_dataset/questions-words.txt'
...
def generate_embed_model_norm_pkl(media_name, random_seed=None, model_type=None):
    ...
    # 3. 保存
    # save_dir = '/home/newsgrid/zhuhua/NHB/data/embedding/word2vec/media_model/' + media_name
    save_dir = '/home/newsgrid/zhuhua/NHB/data/embedding/word2vec/media/' + media_name
...
def load_embed_model_norm_by_mediaName(media_name, random_seed=None, model_type=None, save_type='.bin'):
    ...
    # root_dir = '/home/newsgrid/zhuhua/NHB/data/embedding/word2vec/media/' + media_name
    root_dir = '/home/newsgrid/zhuhua/NHB/data/embedding/word2vec/media_model/' + media_name
    if save_type == '.bin':
